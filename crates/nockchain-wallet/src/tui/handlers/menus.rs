...
use crate::tui::components::menus::{
    BOOL, IMPORT_SRC, KEYS_MENU, NOTES_MENU, SETTINGS_MENU, SIGN_MENU, TX_MENU,
    WATCH_MENU,
};
use crate::tui::create_tx::CreateTxWizard;
use crate::tui::hooks::logging::{log_help, log_verbose_info};
use crate::tui::hooks::terminal::Term;
use crate::tui::screens::{ConfirmThen, TuiControl, Screen, TextThen};
use crate::tui::session::current_api_listen;
use crate::tui::session_client::api_base_url;
use crate::tui::store::UIStore;
use crate::tui::{normalize_slash_cmd, session};
...
pub(super) fn navigate_main_menu_item(store: &mut UIStore, i: usize) {
    let next = match i {
        ...
        7 => crate::tui::prompt_overlay::exit_confirm(Screen::Home, 1),
        _ => Screen::Home,
    };
                    ...
                    match i {
                        ...
                        1 => {
                            super::replace_screen(
                                store,
                                crate::tui::prompt_overlay::text_prompt_screen(
                                    crate::tui::prompt_overlay::prompt_underlay(&taken),
                                    "Child index (u64)",
                                    String::new(),
                        ...
                        7 => {
                            super::replace_screen(
                                store,
                                crate::tui::prompt_overlay::confirm_prompt_screen(
                                    crate::tui::prompt_overlay::prompt_underlay(&taken),
    ...
    match taken {
        Screen::ExitConfirm {
            ...
            Err(()) => {
                super::replace_screen(
                    store,
                    crate::tui::prompt_overlay::exit_confirm((*underlay).clone(), sel),
                );
                Ok(TuiControl::Continue)
            }
            Ok(None) => {
