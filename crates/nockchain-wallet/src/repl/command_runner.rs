...
use crate::create_tx::WrittenTxSnapshot;
...
pub(crate) type NnsLookupCompletion = Result<crate::repl::nns::NnsLookupOk, String>;
...
pub(crate) struct ReplRuntime {
    ...
    /// Captured kernel `%markdown` (create-tx, send-tx, …) for REPL formatters.
    pub repl_markdown_sink: Arc<std::sync::Mutex<String>>,
        ...
        Ok(mut data) => {
            if is_create_tx {
                append_create_tx_event(
                    &mut data.events,
                    &rt.repl_markdown_sink.lock().unwrap(),
                    tx_snapshot_before,
                )
                .await?;
            }
...
pub(crate) fn schedule_wallet_command(
    ...
    tokio::task::spawn_local(async move {
        ...
        let events = outcome
            ...
            .unwrap_or_default();
        let markdown = rt.repl_markdown_sink.lock().unwrap().clone();
...
pub(crate) fn schedule_nockname_resolve(
    ...
    let tx = done_tx.clone();
    tokio::task::spawn_local(async move {
        let nockname = crate::repl::nns::resolve_primary_name(&address)
            .await
            .ok()
            .flatten();
...
pub(crate) fn schedule_home_identity_fetch(
    ...
    tokio::task::spawn_local(async move {
        ...
        let nockname = match address.as_deref() {
            Some(a) => crate::repl::nns::resolve_primary_name(a).await.ok().flatten(),
            None => None,
        };
...
pub(crate) fn schedule_nns_lookup(
    ...
    tokio::task::spawn_local(async move {
        let result = crate::repl::nns::lookup_name(&raw).await;
...
pub(crate) fn schedule_nns_register(
    ...
    let recipient = crate::repl::nns::build_registry_recipient(canonical_name)?;
    let cmd = crate::repl::nns::schedule_create_tx_command(recipient);
